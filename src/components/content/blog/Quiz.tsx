...
export default function Quiz(props: QuizType) {
  ...
  return (
    <div className='prose dark:prose-invert relative mt-8 w-full !max-w-none rounded border p-4 pt-8 dark:border-gray-600'>
      <div className='text-center'>
        <h4 className='md:text-xl text-lg'>{props.question}</h4>
        {props.description && (
          <p className='md:text-sm text-base'>{props.description}</p>
        )}
        {/* <button onClick={() => setSelected(undefined)}>reset</button> */}
      </div>
      <div className='md:grid-cols-2 md:gap-4 mt-4 grid gap-2'>
